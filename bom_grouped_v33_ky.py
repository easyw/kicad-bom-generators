...
"""
    @package
    children of 'bom_grouped_v33.py'
    version r33 - 06.2016  [bom_grouped_v33_ky.py]
"""
...
class component():
            ...
            if self.getLib() == other.getLib():
                if self.getPart() == other.getPart():
                    if self.getFootprint() == other.getFootprint():
                        #logging.warning('%s 1', self.getFootprint());
                        #logging.warning('%s 2', other.getFootprint());
                        #logging.warning('%s 2', self.getField('Voltage'));
                        if self.getField('Manf#') == other.getField('Manf#'):  #maui test
                            if self.getField('Voltage') == other.getField('Voltage'):  #maui test
                                if self.getField('Tolerance') == other.getField('Tolerance'):  #maui test
                                    if self.getField('Power') == other.getField('Power'):  #maui test
                                        if self.getField('Temp') == other.getField('Temp'):  #maui test
                                            if self.getField('Current') == other.getField('Current'):  #maui test
                                                result = True
        return result
    ...
    def getField(self, name):
        ...
        """
        # field = self.element.get("field", "name", name).encode('ascii', 'ignore').decode('ascii')
        field = self.element.get("field", "name", name).encode('utf-8')
        if field == "":
            try:
...
class netlist():
    ...
    def groupComponents(self):
        ...
        groups = sorted(groups, key=lambda group: group[0].getRef()) #maui
        #groups = sorted(groups, key=lambda group: int(group[0].getRef()[1:]))  #maui to complete
        return groups
